...
class SobreFactura(models.Model):  # clase SobreFactura
    _name = "l10n_uy_cfe.sobre_factura"

    ns = fields.Char(string="ns", default="http://efactura.info")
    xsi = fields.Char(string="xsi", default="http://www.w3.org/2001/XMLSchema-instance")
    schemaLocation = fields.Char(string="schemaLocation", default="SobreFactura.xsd")
    root = fields.Char(string="root")

    def getDocument(self, sobre):
        tag = etree.QName(self.ns, "SobreFactura")
        nsmap1 = OrderedDict([("xsi", self.xsi), ("ns", self.ns)])
        schemaLocation = "{%s}%s" % (self.xsi, "schemaLocation")
        self.root = etree.Element(
            tag.text, attrib={schemaLocation: self.schemaLocation}, nsmap=nsmap1
        )
        # self._root.set('schemaLocation', schemaLocation)
        cabezal = etree.SubElement(self.root, "Cabezal")

        etree.SubElement(cabezal, "RUTEmisor").text = sobre.rutEmisor
        etree.SubElement(cabezal, "NumSobre").text = str(sobre.numero)
        etree.SubElement(cabezal, "Fecha").text = (
            sobre.fecha
        )  # datetime.strptime(sobre.send_date, "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%dT%H:%M:%S")

        cfe = etree.SubElement(self.root, "CFESimple")
        # parser = etree.XMLParser(remove_blank_text=True, ns_clean=False)
        xml = CFESimple().getInvoice(
            sobre.cfe
        )  # etree.fromstring(sobre.cfe, parser=parser, base_url=None)
        ...
        sobre = etree.tostring(
            self.root, pretty_print=True, xml_declaration=True, encoding="utf-8"
        )
        return str(sobre, "utf-8").replace(
            '<CFE version="1.0">',
            '<CFE version="1.0" xmlns:ns="http://efactura.info" >',
        )


class CFESimple:
    _ns0 = "http://efactura.info"
    _root = None

    def _getVoucher(self, id_doc, documento):
        tag = etree.QName(self._ns0, "TipoCFE")
        etree.SubElement(id_doc, tag.text, nsmap={"ns0": tag.namespace}).text = (
            documento.tipoCFE
        )
    ...
    def _getCompany(self, emisor, documento):
        ...
        etree.SubElement(emisor, tag.text, nsmap={"ns0": tag.namespace}).text = (
            empresa.numDocumento
        )
        ...
        if empresa.sucursal:
            for sucursal in empresa.sucursal:
                ...
                etree.SubElement(
                    emisor, tag.text, nsmap={"ns0": tag.namespace}
                ).text = str(sucursal.codigo)
    ...
    def _getPartner(self, receptor, documento):
        ...
        etree.SubElement(receptor, tag.text, nsmap={"ns0": tag.namespace}).text = (
            adquirente.tipoDocumento or "3"
        )
        tag = etree.QName(self._ns0, "CodPaisRecep")
        etree.SubElement(receptor, tag.text, nsmap={"ns0": tag.namespace}).text = (
            adquirente.codPais or "UY"
        )
    ...
    def _getTotal(self, totales, documento):
        ...
        etree.SubElement(totales, tag.text, nsmap={"ns0": tag.namespace}).text = (
            documento.moneda
        )
        if documento.tipoCFE in ["121", "122", "123"]:
            tag = etree.QName(self._ns0, "TpoCambio")
            etree.SubElement(totales, tag.text, nsmap={"ns0": tag.namespace}).text = (
                str(documento.tasaCambio)
            )
            tag = etree.QName(self._ns0, "MntExpoyAsim")
            etree.SubElement(totales, tag.text, nsmap={"ns0": tag.namespace}).text = (
                str(documento.mntTotal)
            )
            ...
            etree.SubElement(totales, tag.text, nsmap={"ns0": tag.namespace}).text = (
                str(documento.mntPagar)
            )
        elif documento.tipoCFE in ["182"]:
            if documento.moneda != "UYU":
                tag = etree.QName(self._ns0, "TpoCambio")
                etree.SubElement(
                    totales, tag.text, nsmap={"ns0": tag.namespace}
                ).text = str(documento.tasaCambio)
            ret_perc_vals = {}
            cred_fisc = 0.0
            mnt_retenido = 0.0
    ...
    def _getLines(self, detalle, line, line_number):
        ...
        etree.SubElement(item, tag.text, nsmap={"ns0": tag.namespace}).text = str(
            line_number
        )

        tag = etree.QName(self._ns0, "IndFact")
        etree.SubElement(item, tag.text, nsmap={"ns0": tag.namespace}).text = (
            line.indicadorFacturacion
        )
    ...
    def _getRetencPercepLines(self, detalle, line, line_number):
        ...
        tag = etree.QName(self._ns0, "CodRet")
        etree.SubElement(retenc_precep, tag.text, nsmap={"ns0": tag.namespace}).text = (
            str(line.codigo)
        )
    ...
    def _getRef(self, documento, etck):
        ...
        referencia = etree.SubElement(
            referenciam, tag.text, nsmap={"ns0": tag.namespace}
        )

        tag = etree.QName(self._ns0, "NroLinRef")
        etree.SubElement(referencia, tag.text, nsmap={"ns0": tag.namespace}).text = str(
            len(documento.referencias)
        )
        ...
        for ref in documento.referencias:
            ...
            etree.SubElement(
                referencia, tag.text, nsmap={"ns0": tag.namespace}
            ).text = ref.tipoDocRef
            tag = etree.QName(self._ns0, "Serie")
            etree.SubElement(
                referencia, tag.text, nsmap={"ns0": tag.namespace}
        ...
        if documento.descuentos:
            ...
            for descuento in documento.descuentos:
                ...
                item = etree.SubElement(
                    desc_global, tag.text, nsmap={"ns0": tag.namespace}
                )
