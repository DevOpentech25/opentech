...
class AccountEdiDocument(models.Model):
    ...
    uy_send_number = fields.Integer(
        "Send Number", compute="_compute_uy_send_number", store=True
    )
    uy_send_date = fields.Datetime("Send Date")

    uy_error_code = fields.Selection(
        "_get_uy_error_code", string="Error Code", readonly=True
    )

    uy_document_state = fields.Selection(
        selection=[("AE", "Comprobante aceptado"), ("BE", "Comprobante rechazado")],
    ...
    def get_uy_number(self):
        ...
        seq_id = self.env["ir.sequence"].search(
            [("code", "=", "uy.edi.number"), ("company_id", "=", company_id)],
            limit=1,
            order="company_id",
        )
        if not seq_id:
