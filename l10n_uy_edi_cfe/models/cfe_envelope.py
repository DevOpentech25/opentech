...
class CfeEnvelope(models.Model): # class Sobre
    ...
    documento_id = fields.Many2one("empresa.documento", string="Documento") # self.cfe = Documento(vals.get('documento', {}))
    ...
    def enviarCFE(self):
        ...
        if self.servidor.codigo == 'efactura':
            ...
            cliente =  self.env['einvoice.express.client.soap']._connect()  # FEClient(self.servidor.url) Invoice Express
            FacturaExpress = self.env['einvoice.express']
            xml_data = FacturaExpress().generate_xml(self.cfe) # invoice_express.py FacturaExpress().xmlData(self.cfe)
            vals = {}
    ...
    def obtenerPdfCFE(self, ducument_url_id):
        if self.servidor.codigo == 'biller':
            ...
            cliente = cliente =  self.env['einvoice.express.client.soap']._connect()  # FEClient(self.servidor.url) Invoice Express
