...
class UyStockEdiDocument(models.Model):
    ...
    edi_format_id = fields.Many2one(
        default=lambda self: self.env.ref("l10n_uy_edi_cfe.edi_uy_cfe")
    )
    ...
    def action_sent(self):
        for edi_document_id in self:
            ...
            if uy_check_cfe:
                ...
                edi_document_result = self.env[
                    "uy.edi.send.cfe"
                ].check_cfe_eresuardo_status(edi_document_id, edi_document_id)
                if not edi_document_result.get("estado"):
                    edi_document_result = self.env["uy.edi.send.cfe"].send_eresuardo(
                        edi_document_id
                    )
            else:
                edi_document_result = self.env["uy.edi.send.cfe"].send_eresuardo(
            ...
            if edi_document_result.get("estado"):
                if edi_document_id.picking_id.company_id.uy_server == "biller":
                    ...
                    if edi_document_id.picking_id.company_id.uy_verification_url:
                        date = (
                            edi_document_id.invoice_date or edi_document_id.date
                        ).strftime("%d/%m/%Y")
                        string = "%sconsultaQR/cfe?%s,%s,%s,%s,%s,%s,%s" % (
                            edi_document_id.picking_id.company_id.uy_verification_url,
                            edi_document_id.picking_id.company_id.vat,
                            "181",
                            values.get("uy_cfe_serie"),
                            values.get("uy_cfe_number"),
                            edi_document_id.amount_total,
                            date,
