...
class AccountMove(models.Model):
    ...
    uy_currency_rate = fields.Float(
        "Currency Rate",
        store=True,
        readonly=True,
        compute="_compute_uy_currency_rate",
        copy=False,
        digits=(12, 3),
    )
    uy_tax_min_rate = fields.Float(
        "Tax Minimun Rate", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_tax_basic_rate = fields.Float(
        "Tax Basic Rate", compute="_get_uy_amount_total", store=True, copy=False
    )

    uy_amount_unafected = fields.Float(
        "Unafected", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_tax_min_base = fields.Float(
        "Tax Minimun Base", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_tax_basic_base = fields.Float(
        "Tax Basic Base", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_tax_min = fields.Float(
        "Tax Minimun Amount", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_tax_basic = fields.Float(
        "Tax Basic Amount", compute="_get_uy_amount_total", store=True, copy=False
    )
    uy_amount_untaxed = fields.Float(
        "Amount Untaxed Amount", compute="_get_uy_amount_total", store=True, copy=False
    )

    uy_document_code = fields.Selection(
        selection="_get_uy_invoice_code", string="Invoice Type Code"
    )
    ...
    uy_qr_code = fields.Binary(
        "Qr Code Data", compute="_compute_uy_qr_code", copy=False
    )
    uy_gross_amount = fields.Selection(
        [("0", "Untaxed"), ("1", "Taxes included"), ("2", "Untaxed e-Boleta")],
        "Gross Amount",
        compute="_compute_uy_gross_amount",
        copy=False,
    )

    uy_reversed_entry_id = fields.Many2one(
        "account.move",
        string="Reversal of Credit/Debit Note",
    ...
    uy_sales_mode = fields.Selection(
        selection="_get_uy_sales_mode", string="sales Mode"
    )
    ...
    def _onchange_uy_document_code(self):
        ...
        for move in self:
            l10n_latam_document_type_id = document_type_obj.search(
                [("code", "=", move.uy_document_code)], limit=1
            )
    ...
    def action_check_cfe_pdf_from_moves(self):
        move_ids = self.search(
            [
                ...
                ("state", "=", "posted"),
                ("company_id.uy_server", "in", ["biller"]),
                "|",
                ("uy_print", "=", False),
                ("uy_constancy", "=", False),
            ],
    ...
    def action_retry_edi_documents_error(self):
        res = super(
            AccountMove, self.with_context(uy_check_cfe=True)
    ...
    def action_check_cfe_pdf_status(self):
        for move in self:
            ...
            edi_document_id = (
                len(edi_document_ids) > 1 and edi_document_ids[0] or edi_document_ids
            )
            if edi_document_id:
                if not move.uy_print:
                    ...
                    if res.get("estado") and res.get("respuesta", {}).get("pdf", ""):
                        ...
                        move.message_post(
                            body=_("Error.<br/> %s") % res.get("respuesta", {})
                        )

                if not move.uy_constancy:
                    res = self.env["uy.edi.send.cfe"].get_cfe_invoice_status(
                        move, edi_document_id
                    )
                    if res.get("estado") and res.get("respuesta", {}).get("cae", ""):
                        data = res.get("respuesta", {})
                        if data.get("cae"):
                            vals = {
                                "uy_constancy": data.get("cae", {}).get(
                                    "numero", False
                                ),
                                "uy_constancy_serie": data.get("cae", {}).get(
                                    "serie", False
                                ),
                        ...
                        move.message_post(
                            body=_("CAE the electronic document succeeded.<br/>")
                        )
    ...
    def _cumpute_uy_is_cfe(self):
        for move in self:
            move.uy_is_cfe = bool(
                move.journal_id.edi_format_ids.filtered(
    ...
    def _compute_uy_gross_amount(self):
        for move in self:
            tax_ids = move.invoice_line_ids.mapped("tax_ids").filtered(
                lambda s: s.amount > 0.0
            )
            if move.uy_document_code not in ["151", "152", "153"]:
                if tax_ids and len(tax_ids) == len(
                    tax_ids.filtered(lambda s: s.price_include == True)
                ):
                    move.uy_gross_amount = "1"
                elif tax_ids and len(tax_ids) == len(
                    tax_ids.filtered(lambda s: s.price_include == False)
                ):
    ...
    def _compute_uy_edi_detail(self):
        for move in self:
            ...
            uy_cfe_serie = move.edi_document_ids.filtered(
                lambda s: s.edi_format_id.code == "edi_uy_cfe"
            )
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id.uy_doc_type == "2" and self.move_type in [
            "out_invoice",
            "in_invoice",
        ]:
            self.uy_document_code = "111"
        elif self.partner_id.uy_doc_type != "2" and self.move_type in [
            "out_invoice",
            "in_invoice",
        ]:
        ...
        if journal_id.uy_document_code != self.uy_document_code:
            journal_id = self.env["account.journal"].search(
                [
                    ("uy_document_code", "=", self.uy_document_code),
    ...
    def _check_update_uy_sequence(self):
        for move in self.filtered(lambda s: s.edi_document_ids):
            if (
                move.uy_cfe_serie
                and move.uy_cfe_number
                and move.name != "%s-%s" % (move.uy_cfe_serie, move.uy_cfe_number)
            ):
    ...
    def _get_uy_amount_total(self):
        ...
        for move in self:
            ...
            for line in tax_lines:
                res.setdefault(
                    line.tax_line_id.uy_tax_type, {"base": 0.0, "amount": 0.0}
                )
                res[line.tax_line_id.uy_tax_type][
                    "amount"
    ...
    def button_cancel_posted_moves(self):
        acepted_documents = self.mapped("edi_document_ids").filtered(
            lambda s: s.state == "sent"
        )
        if acepted_documents:
            raise ValidationError(
                _("The document cannot be canceled, you must issue a credit note")
            )
    ...
    def uy_import_message_post(self, pdf_file):
        ...
        self.message_post(
            body=_("Sending the electronic document succeeded.<br/>"),
            attachments=[("%s.pdf" % self.name, decodebytes(pdf_file.encode("utf-8")))],
        )
        return True
...
class AccountMoveLine(models.Model):
    ...
    uy_invoice_indicator = fields.Selection(
        selection="_get_uy_invoice_indicator", string="Invoice Indicator"
    )
    uy_amount_discount = fields.Monetary(
        ...
        for line in self:
            ...
            if line.tax_ids and line.discount != 0.0:
                ...
                if line.move_id.uy_gross_amount == "1":
                    line.uy_amount_discount = (
                        taxes_res["total_included"] - line.price_total
                    )
                else:
                    line.uy_amount_discount = (
                        taxes_res["total_excluded"] - line.price_subtotal
                    )
            else:
                line.uy_amount_discount = False
