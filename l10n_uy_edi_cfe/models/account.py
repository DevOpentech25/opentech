...
class AccountJournal(models.Model):
    ...
    uy_credit_note_id = fields.Many2one(
        comodel_name="account.journal",
        string="Credit Note",
        domain="[('type','in', ['sale', 'purchase'])]",
    )
    uy_dedit_note_id = fields.Many2one(
        comodel_name="account.journal",
        string="Debit Note",
        domain="[('type','in', ['sale', 'purchase'])]",
    )
    ...
    uy_server = fields.Many2one(
        "l10n_uy_cfe.server", string="Server", related="company_id.uy_server"
    )
    uy_efactura_print_mode = fields.Selection(
        selection="get_uy_efactura_print_mode", string="Print Mode"
    )
    uy_is_edi_document = fields.Boolean(
        "Is EDI Document", compute="_compute_uy_is_edi_document", store=True
    )

    @api.depends("edi_format_ids")
    def _compute_uy_is_edi_document(self):
        for journal_id in self:
            journal_id.uy_is_edi_document = bool(
                journal_id.edi_format_ids.filtered(lambda s: s.code == "edi_uy_cfe")
            )
