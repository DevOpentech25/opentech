...
class UyCfeWizard(models.TransientModel):
    ...
    uy_username = fields.Char(
        "Username", related="company_id.uy_username", readonly=False
    )
    uy_password = fields.Char(
        "Password/Token", related="company_id.uy_password", readonly=False
    )
    uy_server_url = fields.Char(
        "Server URL", related="company_id.uy_server_url", readonly=False
    )
    uy_server = fields.Many2one(
        "l10n_uy_cfe.server", string="Server", required=True, readonly=False
    )
    uy_efactura_print_mode = fields.Selection(
        selection="get_uy_efactura_print_mode", string="Print mode", readonly=False
    )
    ...
    def default_get(self, fields_list):
        ...
        if (
            self.env.context["active_model"] == "res.config.settings"
            and "active_id" in self.env.context
        ):
            active_settings_ids = self.env["res.config.settings"].browse(
                self.env.context["active_id"]
            )
            company_id = (
                active_settings_ids and active_settings_ids[0].company_id or False
            )
            ...
            values["uy_efactura_print_mode"] = (
                company_id and company_id.uy_efactura_print_mode or False
            )
            ...
            values["uy_verification_url"] = (
                company_id and company_id.uy_verification_url or False
            )
